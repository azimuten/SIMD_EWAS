...
manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
   ...
   d$index = rep.int(seq_along(unique(d$CHR)), times = tapply(d$SNP,d$CHR,length))  # replcace the for loop of line 92-96 to improve efficiency
    ...
    def_args <- list(xaxt='n', bty='n', xaxs='i', yaxs='i', las=1, pch=19,
    ...
    if (nchr==1) {
	    ...
	    points(d[d$index==i,"pos"], d[d$index==i,"logp"], col=col[icol], pch=19, ...)
            icol=icol+1
        }
    }
    ...
    if (!is.null(highlight)) {
        ...
        with(d.highlight, points(pos, logp, col="green3", pch=19, ...)) 
    }
